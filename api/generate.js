...
const PREMIUM_CHAT_DAILY_LIMIT = 50;
...
async function checkPremiumToken(premiumToken){
...
export default async function handler(req, res) {
  ...
  const { mode, userRequest, sectionHeading, type, isContinuation, useFallbackModel, provider, startingQuestionNumber, questionCount, markScheme, premiumToken, continueFrom, chatHistory } = req.body || {};
  ...
  // it shouldn't compete with the "3 documents/day" budget.
  if (mode === 'chat') {
    const ip = getClientIp(req);
    const isPremium = await checkPremiumToken(premiumToken);
    const chatLimit = isPremium ? PREMIUM_CHAT_DAILY_LIMIT : CHAT_DAILY_LIMIT;
    const limitResult = await checkAndIncrementLimit(`chat:${ip}`, chatLimit);
    if (!limitResult.allowed) {
      return res.status(429).json({ error: `Daily doubt-chat limit reached (${chatLimit} per day). ${isPremium ? '' : 'Upgrade to Premium for more, or '}try again after midnight.` });
    }
  ...
  if (!isContinuation) {
    ...
    const ip = getClientIp(req);
    const limitResult = await checkAndIncrementLimit(ip, limit);
    if (!limitResult.allowed) {
